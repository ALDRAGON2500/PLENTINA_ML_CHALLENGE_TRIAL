...
#function to group data within the step,nameOrig,nameDest,type
def group_df(df):
    col_list=['step',
     'type',
     'amount',
    ...
    new_df[['step','type','amount','nameOrig','nameDest']]=df.groupby(['step','nameOrig','nameDest','type']).sum().reset_index()[['step','type','amount','nameOrig','nameDest']]

    new_df[['oldbalanceOrig','oldbalanceDest']]=df.groupby(['step','nameOrig','nameDest','type']).first().reset_index()[['oldbalanceOrig','oldbalanceDest']]
    
    new_df[['newbalanceOrig','newbalanceDest']]=df.groupby(['step','nameOrig','nameDest','type']).last().reset_index()[['newbalanceOrig','newbalanceDest']]
...
def fraud_detection():
    ...
    y_pred=transform_data(group_df(result))
